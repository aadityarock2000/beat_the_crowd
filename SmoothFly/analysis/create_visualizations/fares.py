...
>>>>>>> main
    ...
    consumer_fares['perc_sh_passengers'] = round((consumer_fares['SHPax']/
                                                  consumer_fares['TotalFaredPax']), 4)
    consumer_fares['perc_lh_passengers'] = round((consumer_fares['LHPax']/
                                                  consumer_fares['TotalFaredPax']), 4)

    return(consumer_fares) # pylint: disable=superfluous-parens
...
def prem_disc_by_year():
    ...
    fig = px.bar(prem_disc.rename(columns =
                                  {"Short_haul_percent_premium":"Short Haul Percent Discount",
...
def avg_fare_by_city():
    ...
    fig.update_layout(
            updatemenus=[{"buttons": [{"label": k,"method": "update",
                    # list comprehension for which traces are visible
                    "args": [{"visible": [kk == k for kk in figs.keys()]},
                             {"title":go.layout.xaxis.Title(
                                     text=f"Percent of Passengers flying vs. Average Airfare Prices <br><sup>{k} </sup>" # pylint: disable=line-too-long
                                                  )}],
    }for k in figs.keys()]}])
...
def pct_by_avg_fare():
    ...
    figs = {
            c: px.area(consumer_fares[consumer_fares['year'] ==c].rename(
                    columns = {"perc_sh_passengers":"Short Haul",
                               "perc_lh_passengers":"Long Haul" }),
    ...
    fig.update_layout(updatemenus=[dict(
            buttons = list([{"label": k,"method": "update",
                             # list comprehension for which traces are visible
                            "args": [{"visible": [kk == k for kk in figs.keys()]},
                                     {"title":go.layout.xaxis.Title(
                                             text=f"Percent of Passengers Flying by Average Fare Price <br><sup>{k} </sup>"# pylint: disable=line-too-long
                                                          )}],}
    for k in figs.keys()
    ]), x = 1)])
